...
namespace Scrabble
{
    class Arbol
    {
        ...
        public Nodo Insertar(Informacion[,] Info, string palabra, int direccion, int Inicio_i_padre, int Inicio_j_padre, int puntaje, Nodo NuevoNodo)
        {
            ...
            if (NuevoNodo.Hijo == null) //Si no tenia ningun nodo hijo, lo insertarmos como hijo.
            {
                ...
                temp.Inicio_i_padre = Inicio_i_padre;
                temp.Inicio_j_padre = Inicio_j_padre;
                temp.PalabraPadre = palabra;
        ...
        public Nodo Buscar(int MayorPuntaje,Nodo nodo)
        {
            Nodo buscado = null;
            if (nodo == null)
                return buscado;
            if (nodo.Puntaje==MayorPuntaje)
            {
                buscado = nodo;
                return buscado;
            }
            ...
            if (nodo.Hijo!=null)
            {
                buscado = Buscar(MayorPuntaje, nodo.Hijo);
                if (buscado != null)
                    return buscado;
            }
            //Los hermanos
            ...
            {
                buscado = Buscar(MayorPuntaje, nodo.Hermano);
                if (buscado != null)
                    return buscado;
            }
            return buscado;
